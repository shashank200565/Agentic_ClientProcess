...
export function AutomationBlueprintPage() {
  ...
  return <><section className="mx-auto max-w-6xl"><Link to={`/review/${workflowId}`} className="text-sm font-semibold text-teal-700">← Back to step review</Link><p className="mt-8 text-sm font-semibold uppercase tracking-[0.22em] text-teal-700">03 / Automation blueprint</p><h1 className="mt-3 max-w-3xl font-display text-4xl font-semibold tracking-tight text-[#102a2c]">{blueprint.current_step.name}</h1><p className="mt-3 text-slate-500">A constrained, conventional path for a step judged suitable for automation.</p><div className="mt-8 grid gap-4 rounded-3xl border border-slate-200 bg-white p-6 lg:grid-cols-2"><ComparisonColumn label="Before" title={blueprint.current_step.name} text={blueprint.current_step.description} /><ComparisonColumn label="After · automation blueprint" title={blueprint.mechanism_type.replaceAll("_", " ")} text={[blueprint.trigger_condition, blueprint.rationale].join(". ")} /></div><div className="mt-8 grid gap-4 sm:grid-cols-3"><Info label="Mechanism" value={blueprint.mechanism_type.replaceAll("_", " ")} /><Info label="Trigger" value={blueprint.trigger_condition} /><Info label="Rationale" value={blueprint.rationale} /></div><div className="mt-8"><WorkflowDiagram diagram={blueprint.diagram} /></div></section><ScreenChat context={storedScreenContext(workflowId, stepId, blueprint)} /></>;
}
...
function ComparisonColumn({ label, title, text }: { label: string; title: string; text: string }) { return <div className={`rounded-2xl p-5 ${label === "Before" ? "bg-slate-50" : "bg-[#e4eee9]"}`}><p className="text-xs font-bold uppercase tracking-[0.16em] text-teal-700">{label}</p><h2 className="mt-2 font-display text-xl font-semibold text-[#102a2c]">{title}</h2><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-700">{textBullets(text).map((item, index) => <li key={`${label}-${index}`} className="flex gap-2"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-teal-600" />{item}</li>)}</ul></div>; }
